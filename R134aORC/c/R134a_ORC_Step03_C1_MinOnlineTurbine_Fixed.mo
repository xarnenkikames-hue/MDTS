model R134a_ORC_Step03_C1_MinOnlineTurbine_Fixed
  "C1：修正后的最小在线透平接入（消除蒸发器出口零流三通）"
  ...
  // 4. 修正后的在线透平接入拓扑
  // 重点：
  // - 蒸发器出口先接短管，再进高压 header
  // - 两支路在低压 header 汇流，再经短管回冷凝器
  // - 仅保留透平入口阀，不再用出口阀
  // ======================================================================

  // 高压侧：蒸发器出口到分流 header 之间放一个短 S 元件
...
end R134a_ORC_Step03_C1_MinOnlineTurbine_Fixed;