model R134a_ORC_Step04_D0_LoadFromC1
  "D0：基于最终C1母版的透平加负荷基准模型（仅增加机械负荷）"
  ...
  annotation(
    ...
    experiment(Algorithm=Dassl,StartTime=0,StopTime=20,Tolerance=0.0001,InlineIntegrator=false,InlineStepSize=false,NumberOfIntervals=2000,StoreEventValue=0));

  // ======================================================================
  // A. 冷态初始化（完全沿用成功 C1 母版）
  // ======================================================================
  ...
  // B. 冻结：成功热态母版参数
  // ======================================================================
  parameter Real valve_k = 0.5 "主阀固定开度";
  ...
  parameter Real hotFlowStart = 100 "热源流量启动时间";
  parameter Real hotFlowOffset = 0.0 "热源初始流量";
  parameter Real hotFlowHeight = 0.50 "热源增量";
  parameter Real hotFlowDuration = 60 "热源流量爬升时长";

  parameter Real hotTStart = 150 "热源温度启动时间";
  parameter Real hotTOffset = 293.15 "热源初始温度";
  parameter Real hotTHeight = 10.0 "热源升温增量（到 303.15 K）";
  parameter Real hotTDuration = 150 "热源升温时长";

  // ======================================================================
  // C. D0 新增：机械负荷参数
  // ======================================================================
  parameter Real turbineValveStart = 500 "透平入口阀开始开启时刻";
  parameter Real turbineValveDuration = 50 "透平入口阀开启历时";
  parameter Real turbineValveFinal = 0.5 "透平入口阀最终开度";

  parameter Real loadStart = 800 "外部负荷开始接入时刻";
  parameter Real loadDuration = 80 "外部负荷爬升时间";
  parameter SI.Torque loadTorqueFinal = -0.02 "最终负荷扭矩（负值表示吸收功）";
...
end R134a_ORC_Step04_D0_LoadFromC1;