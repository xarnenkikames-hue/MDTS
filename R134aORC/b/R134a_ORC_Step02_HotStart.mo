model R134a_ORC_Step02_HotStart"开启热源"

  import SI = Modelica.SIunits;

  annotation(
    __MWORKS(version="26.1.3",ContinueSimConfig(SaveContinueFile="false",SaveBeforeStop="false",NumberBeforeStop=1,FixedContinueInterval="false",ContinueIntervalLength=40,ContinueTimeVector)),
    Diagram(coordinateSystem(extent={{-100,-100},{100,100}}, grid={2,2})),
    experiment(Algorithm=Dassl,StartTime=0,StopTime=1000,Tolerance=0.0001,InlineIntegrator=false,InlineStepSize=false,NumberOfIntervals=500,StoreEventValue=0));
  ...
  parameter Real hotFlowStart = 100 "热源流量启动时间（>StopTime 表示禁用）";
  parameter Real hotFlowOffset = 0.0 "热源初始流量";
  parameter Real hotFlowHeight = 0.55 "热源增量";
  parameter Real hotFlowDuration = 60 "热源流量爬升时长";

  parameter Real hotTStart = 150 "热源温度启动时间（>StopTime 表示禁用）";
  parameter Real hotTOffset = 303.15 "热源初始温度";
  parameter Real hotTHeight = 25.0 "热源升温增量";
